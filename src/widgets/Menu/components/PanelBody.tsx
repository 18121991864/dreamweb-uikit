...
const StyledSvgImg = styled.img<{ size: string }>`
  width: ${({ size }) => size};
  height: ${({ size }) => size};
  /* width: 20px;
  height: 20px; */
      ...
      {links.map((entry) => {
        ...
        const src =
          entry.href === location.pathname
            ...
            : `/images/menu/dark/${entry.icon}.svg`;
        const iconElement = <StyledSvgImg src={src} size={isLand && (entry.icon!== 'Home') ? '30px' : '20px'}  />;
        ...
        return (
          <MenuEntry
            ...
            className={calloutClass}
          >
            <MenuLink href={entry.href} style= {{pointerEvents: entry.disabled? 'none': 'auto'}} onClick={handleClick}>
              {iconElement}
